 ...
 *     this.render("views/tasks/click.ejs")                  // renders with views/tasks/click.ejs to this.element using this
 *   }
 * })
 * @code_end
 ...
 * <td>locals</td>
 * <td>{}</td>
 * <td>If present, fields of the locals will be copied to data object passed to view</td>
 * </tr>
 * </tbody></table>
 *
 */
    ...
    if (typeof options == 'string') {
        ...
        var convert = function (url) {
            var url = MVC.String.include(url, '/') ? url.split('/').join('/_') : controller_name + '_controller/' + url;
        ...
        var template;
        if (options.action) {
            template = convert(options.action);
        } else if (options.partial) {
            template = convert(options.partial);
        } else {
            template = controller_name + '_controller/' + action_name.replace(/\.|#/g, '').replace(/ /g, '_') + MVC.View.ext;
        }
        ...
        if (!plugin_url) {
            var url;
            try {
                url = '../views/' + MVC.app_name + '/' + template;
            ...
            view = new MVC.View({url: plugin_url});
        }
    ...
    for (var l = 0; l < locations.length; l++) {
        ...
        if (options[locations[l]]) {
            ...
            if (locations[l] == 'to') {
                ...
                if (!MVC.$E.insert) throw {
                    message: "Include can't insert " + locations[l] + " without the element plugin.",
                    name   : 'ControllerView: Missing Plugin'
                };
                var opt = {};
