...
{ 
    ...
    /**
     *
     * @param {Function|Object} el filter function or object to remove
     * @returns {Array}
     */
    remove : function(a,el){
    var result = [];
    for(var i = 0, size = a.length; i < size; ++i){
        if(typeof(el) == 'function' && el(a[i])){
            continue;
        } else if(a[i] == el){
            continue;
        } else {
...
MVC.Array.last = function () {
    if (this.length == 0)
        throw "Array: querying zero length array";
    if (this.length == 1)
        return this[0];
    return this[this.length - 1];
};

MVC.Array.first = function () {
    if (this.length == 0)
        throw "Array: querying zero length array";
    return this[0];
};

MVC.Array.findFirst = function (condition) {
    if (this.length == 0)
        throw "Array: querying zero length array";
...
/**
 * Performs binary search of the find in this Array
 *
 * @param {*} find
 * @param {Function} comparator :=(el, ndx, find)
 * @return {*} element or null
 */
MVC.Array.binarySearch = function (find, comparator) {
    var low = 0, high = this.length - 1,
        i, comparison;
    while (low <= high) {
        i = Math.floor((low + high) / 2);
        comparison = comparator(this[i], i, find);
        if (comparison < 0) {
            low = i + 1;
            continue;
        }
...
if(!MVC._no_conflict){
    ...
    Array.binarySearch = MVC.Array.binarySearch;
}