MVC.ModelViewHelper = MVC.Class.extend(
{
    init: function(){
        ...
        if(!(modelClass = this.modelClass = window[this.className]) )
...
},
{
    ...
    show_errors : function(){
        ...
        for(var i=0; i< this._inst.errors.length; i++){
            ...
            for(var error_attr in error){
                if(!error.hasOwnProperty(error_attr)) continue;
                var el = MVC.$E(this._className+"_"+error_attr);
                if(el){
                    el.className="error";
                    var er_el = MVC.$E(this._className+"_"+error_attr+"_error" );
                    if(er_el) er_el.innerHTML = error[error_attr].toString();
                }
                else
                    errs.push(error_attr+' is '+error[error_attr].toString());
            }
		}
        if(errs.length > 0){
             if(err) err.innerHTML = errs.join("<br/>");
