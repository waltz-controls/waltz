Plugin =  function(uri, name){
    this.repository_url = "https://bitbucket.org/ingvord/jmvc-plugins/raw/default/";
    if(! uri.match(/^http/)){
        this.name = uri;
...
Plugin.prototype = {
    install_using_http: function(options){
        ...
        var fetcher = new RecursiveHTTPFetcher(this.uri, -1, "jmvc/plugins/"+this.name, false ,this.repository_url);
    ...
    check_plugin_list : function(){
        ...
        var plugin_list_source = readUrl(this.repository_url + "jmvc/rhino/command/plugin_list.json");
        var plugin_list;
